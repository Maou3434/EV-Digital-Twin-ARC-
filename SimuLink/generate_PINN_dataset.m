    ...
    for t = 1:length(tempsC)
        ...
        for s = 1:length(socsPct)
            ...
            for m = 1:length(massScales)
                ...
                Temperature_C = ...
                    Batt.BattTemp.Data(:) - 273.15;

                AmbientTemp_C = ...
                    repmat(tempC,length(Time_s),1);

                InitialSOC_pct = ...
                    repmat(socPct,length(Time_s),1);

                InitialBattTemp_C = ...
                    repmat(tempC,length(Time_s),1);

                MassScale = ...
                    repmat(massScale,length(Time_s),1);

                DriveCycle = ...
                    repmat(string(cycleName),length(Time_s),1);
