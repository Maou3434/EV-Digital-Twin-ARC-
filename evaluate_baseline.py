...
def run_evaluation(model_path, config=None, device=None):
    """
    Core function to run single-step evaluation.
    Can be called directly from other python scripts.
    """
    logger.info(f"Starting single-step evaluation using model: {model_path}")
    ...
    test_sampler = FileGroupedBatchSampler(test_dataset, batch_size=mlp_cfg['batch_size'], shuffle=False)
    ...
    logger.info("Running single-step predictions...")
    with torch.no_grad():
        for batch_X, batch_y, _ in test_loader:
            ...
            # Reconstruct physical features of index 0 (Temperature_C)
            x_phys = batch_X.cpu().numpy() * scaler_X.scale_ + scaler_X.mean_
    ...
    logger.info("Single-step Absolute Temperature Metrics:")
    ...
    logger.info(f"Single-step evaluation complete in {duration:.1f} seconds.")
    log_system_metrics(logger, prefix="Evaluation Final Resource Usage")
    return metrics

def main():
    parser = argparse.ArgumentParser(description="Evaluate Baseline MLP Model")
    ...
    try:
        ...
        logger.error(f"Single-step evaluation failed: {e}", exc_info=True)
        raise e
